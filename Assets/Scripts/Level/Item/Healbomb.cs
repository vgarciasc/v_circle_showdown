...
public class Healbomb : MonoBehaviour, ISmashable {
    [SerializeField]
    float lifetime = 2f;
    ...
    IEnumerator blink() {
        ...
        while (true) {
            toggle = !toggle;
            if (toggle) sprite.GetComponent<SpriteRenderer>().color = Color.white;
    ...
    void explode() {
        ...
        GameObject aux = Instantiate(healCloud);
        aux.GetComponentInChildren<HealbombCloud>().set_duration(lifetime);
